...
function buildHouseTour({physics,plan,model,position,config=CASA_TOUR_CONFIG}) {
  ...
  function smooth(raw,ground=false){
    ...
    for(let i=1;i<raw.length-1;i++){
      ...
      if(ground&&physics.doors.some(d=>Math.hypot(b.x-d.hingeX,b.z-d.hingeZ)<d.width+1)){result.push(b);continue;}
  ...
  function rawRoute(from,to){
    if(!aerial(from)&&!aerial(to)){
      const first=groundRoute(from,to.approach||to);if(!first)return null;
      const reveal=to.approach?groundRoute(to.approach,to):null;if(to.approach&&!reveal)return null;
      const result=smooth(reveal?[...first,...reveal.slice(1)]:first,true);
      if(to.bounds)for(const p of result)if(p.x>to.bounds.minX+.15&&p.x<to.bounds.maxX-.15&&p.z>to.bounds.minZ+.15&&p.z<to.bounds.maxZ-.15)p.view=to.focus;
      return result;
    }
...
function createHouseTour({getPhysics,getPlan,getModel,config=CASA_TOUR_CONFIG,onChange=()=>{},plannerFactory=()=>null}) {
  ...
  function update(position,delta){
    ...
    if(!aim){
      ...
      else setRoute(state.index+1,position,navigation.legs[state.index],true);
      return true;
    }
    ...
    const framing=Math.max(0,Math.min(1,1-left/2));if(!aim.view)heading+=difference(destination.heading,heading)*framing;
    ...
    const cornerSpeed=Math.max(.7,cruise*(1-error/Math.PI*.5));
