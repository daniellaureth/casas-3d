...
function buildHouseTour({physics,plan,model,position,config=CASA_TOUR_CONFIG}) {
  ...
  function aerial(p){
    ...
    return p.kind==='aerial'||(!indoors&&p.y>physics.floorAt(p.x,p.z)+eye+.25);
  }
  ...
  function flightClear(a,b,pad=.08){
  ...
  function groundRoute(from,to){
    ...
    if(raised||physics.blockedForTour(from.x,from.z,true,eye)){
      ...
      for(const {p} of candidates){
        if(!flightClear(from,p,.025))continue;
        ...
        for(let i=0;i<=n&&clearDoors;i++){
          ...
          clearDoors=!physics.doors.some(d=>y>(d.bottom??0)&&y<(d.top??2.3)&&physics.hitsDoor(x,z,d,d.openAngle,.025));
        }
        if(!clearDoors)continue;const rest=groundRoute(p,to);
